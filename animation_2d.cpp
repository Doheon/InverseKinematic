...
float ntheta1 = 0.5, ntheta2 = 0.5, ntheta3 = 0.5;
...
-1.0f, 0.0f, -1.0f, 0.0f,  1.0f, 0.0f,
	...
	while (!glfwWindowShouldClose(window))//루프가 시작될 때마다 종료하도록 지시되었는지 확인, 그렇다면 true반환
	{
		...
		linkjoint a1(theta1, 3);
		linkjoint a2(theta2, 3);
		linkjoint a3(theta3, 3);

		a1.insertnode(&a2);
		a2.insertnode(&a3);

		drawnode(a1, spline, elindices.size());


		linkjoint b1(ntheta1, 3);
		linkjoint b2(ntheta2, 3);
		linkjoint b3(ntheta3, 3);

		b1.insertnode(&b2);
		b2.insertnode(&b3);
		drawnode(b1, spline, elindices.size());
...
void mouse_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	if (!movemode) {
		...
		if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
			...
			linkjoint a1(ntheta1, 3);
			linkjoint a2(ntheta2, 3);
			linkjoint a3(ntheta3, 3);
			...
			//vec3 deltapos(-1, 1, 0);
			vec2 origin(0.0f);
			vec2 endpoint = vec2(a3.model * vec4(origin, 0.0f, 1.0f));
			vec2 j1 = origin;
			vec2 j2 = vec2(a1.model * vec4(origin, 0.0f, 1.0f));
			vec2 j3 = vec2(a2.model * vec4(origin, 0.0f, 1.0f));

			vec2 p1 = endpoint - j1;
			vec2 p2 = endpoint - j2;
			vec2 p3 = endpoint - j3;

			p1 = vec2(cross(vec3(0, 0, 1), vec3(p1, 0)));
			p2 = vec2(cross(vec3(0, 0, 1), vec3(p2, 0)));
			p3 = vec2(cross(vec3(0, 0, 1), vec3(p3, 0)));


			deltapos = vec3(xoffset, yoffset, 0);


			mat3x2 jacob = mat3x2(p1, p2, p3);
			mat2x3 invjacob = transpose(jacob) * inverse(jacob * transpose(jacob));
			vec3 deltatheta = invjacob * deltapos;

			ntheta1 += deltatheta.x;
			ntheta2 += deltatheta.y;
			ntheta3 += deltatheta.z;
		}
		else if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_RELEASE)
		{
			firstmouse = true;
