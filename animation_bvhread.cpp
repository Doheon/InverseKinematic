...
-1.0f, 0.0f, -1.0f, 0.0f,  1.0f, 0.0f,
	...
	linkjoint pelvis(0, 1, 22, tr, sc);

	linkjoint lfemurz(0, 3, 0, tr, sc);
	linkjoint lfemurx(0, 1, 0, tr, sc);
	linkjoint lfemury(0, 2, 22, tr, sc);

	linkjoint ltibiaz(0, 3, 0, tr, sc);
	linkjoint ltibiax(0, 1, 0, tr, sc);
	linkjoint ltibiay(0, 2, 22, tr, sc);

	linkjoint lfootz(0, 3, 0, tr, sc);
	linkjoint lfootx(0, 1, 0, tr, sc);
	linkjoint lfooty(0, 2, 22, tr, sc);

	linkjoint ltoesz(0, 3, 0, tr, sc);
	linkjoint ltoesx(0, 1, 0, tr, sc);
	linkjoint ltoesy(0, 2, 22, tr, sc);

	pelvis.insertnode(&lfemurz);
	lfemurz.insertnode(&lfemurx);
	lfemurx.insertnode(&lfemury);
	lfemury.insertnode(&ltibiaz);
	ltibiaz.insertnode(&ltibiax);
	ltibiax.insertnode(&ltibiay);
	ltibiay.insertnode(&lfootz);
	lfootz.insertnode(&lfootx);
	lfootx.insertnode(&lfooty);
	lfooty.insertnode(&ltoesz);
	ltoesz.insertnode(&ltoesx);
	ltoesx.insertnode(&ltoesy);
	...
	while (!glfwWindowShouldClose(window))//루프가 시작될 때마다 종료하도록 지시되었는지 확인, 그렇다면 true반환
	{
		...
		drawnode(&pelvis, spline, elindices.size());
