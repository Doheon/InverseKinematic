...
float posgradient(float deltatheta, linkjoint* chnode, linkjoint* endnode);
...
-1.0f, 0.0f, -1.0f, 0.0f,  1.0f, 0.0f,
	...
	allnode.push_back(&larm1_3);
	allnode.push_back(&larm2);
	allnode.push_back(&rarm1_3);
	allnode.push_back(&rarm2);

	allnode.push_back(&lleg1_3);
	allnode.push_back(&lleg2);
	allnode.push_back(&rleg1_3);
	allnode.push_back(&rleg2);
	...
	while (!glfwWindowShouldClose(window))//루프가 시작될 때마다 종료하도록 지시되었는지 확인, 그렇다면 true반환
	{
		...
		diffusecolor = glm::vec3(10.0f, 10.0f, 10.0f);
		glUniform3fv(glGetUniformLocation(spline.ID, "material.diffuse"), 1, &diffusecolor[0]);

		model = mat4(1.0f);
		model[3][0] = allnode[jointnum]->modelmat()[3][0];
		model[3][1] = allnode[jointnum]->modelmat()[3][1];
		model[3][2] = allnode[jointnum]->modelmat()[3][2];
		model = glm::scale(model, glm::vec3(3.0f));

		diffusecolor = glm::vec3(6.0f, 3.0f, 3.0f);
...
void movenode(linkjoint* node) {
	...
	if (length(goalpos - endpoint) < len && length(goalpos - endpoint) > 1) {
		len = length(goalpos - endpoint);
		...
		for (int i = 0; i < size; i++) {
			theta[i] += 0.02 * posgradient(0.01, chnode[i], node) * (size - i);
			if (theta[i] > 1.5708)
				theta[i] = 1.5708;
			if (theta[i] < -1.5708)
				theta[i] = -1.5709;
		...
		endpoint = vec3(node->modelmat() * vec4(0, 0, 0, 1.0f));
		
	}
}

float posgradient(float deltatheta, linkjoint* chnode, linkjoint* endnode) {
	float fvec = length(vec3(endnode->modelmat() * vec4(0, 0, 0, 1)) - goalpos);
	float ftheta = chnode->theta;
	chnode->changetheta(ftheta + deltatheta);
	float svec = length(vec3(endnode->modelmat() * vec4(0, 0, 0, 1)) - goalpos);
	chnode->changetheta(ftheta);
	return fvec - svec;
}
